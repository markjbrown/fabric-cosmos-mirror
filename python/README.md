...
4. **A Cosmos DB connection in the Fabric portal** — the Fabric REST API does not currently support creating OAuth2 connections programmatically. Create one manually via **Settings → Manage connections and gateways** in the Fabric portal and pass its display name via `--connection`.
...
Required parameters only. Uses DefaultAzureCredential. Mirrors all containers in the database.
...
Only mirror specific containers. Create the Mirror database artifact in a specific folder in the Workspace (folder must exist).
...
Configures the Cosmos DB account for Mirroring when using Virtual Networking or
Private Link. Configures RBAC policies on the Cosmos account, configures network settings, creates the mirror, starts replication, then restores the original network settings.
...
├── auth.py            # Credential building, token acquisition, JWT parsing
├── fabric_client.py   # Constants, HTTP helpers, workspace/connection/folder lookup
├── mirroring.py       # Mirrored database creation, start replication
├── vnet.py            # VNet orchestration, account state capture/restore
├── rbac.py            # Cosmos SQL RBAC role creation and assignment
├── service_tags.py    # Azure service tag download, IP extraction, firewall config
├── network_acl.py     # Network ACL bypass capability and workspace registration
...
- **OAuth2 connections cannot be created programmatically** — the Fabric REST API does not support the OAuth2 credential type. Connections must be created manually in the Fabric portal before using this utility. See the Go implementation's [`futures/CONNECTION_CREATION.md`](../go/futures/CONNECTION_CREATION.md).
