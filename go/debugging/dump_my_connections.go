...
func main() {
	...
	filter := flag.String("filter", "", "Case-insensitive substring filter on connection type (optional)")
	flag.Parse()
	...
	for url != "" {
		...
		for _, raw := range page.Value {
			if *filter != "" && !strings.Contains(strings.ToLower(string(raw)), strings.ToLower(*filter)) {
				continue
			}
			var obj interface{}
